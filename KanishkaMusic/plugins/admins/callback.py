...
from KanishkaMusic import YouTube, app
from KanishkaMusic.core.call import Kanishka
from KanishkaMusic.misc import SUDOERS, db
from KanishkaMusic.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from KanishkaMusic.utils.decorators.language import languageCB
from KanishkaMusic.utils.formatters import seconds_to_min
from KanishkaMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from KanishkaMusic.utils.stream.autoclear import auto_clean
from KanishkaMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Kanishka.pause_stream(chat_id)
        ...
        await CallbackQuery.answer()
        await Kanishka.stop_stream(chat_id)
        ...
        await Kanishka.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Kanishka.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Kanishka.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Kanishka.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Kanishka.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
